...
let request_handler (_ : Socket.Address.Inet.t) =
  Httpaf_examples.Server.echo_post

let error_handler (_ : Socket.Address.Inet.t) =
  Httpaf_examples.Server.error_handler

let main port max_accepts_per_batch () =
  let where_to_listen = Tcp.Where_to_listen.of_port port in
  Tcp.(
    Server.create_sock ~on_handler_error:`Raise ~backlog:10_000
      ~max_connections:10_000 ~max_accepts_per_batch where_to_listen)
...
let () =
  Command.async ~summary:"Echo POST requests"
    Command.Param.(
      map
        (both
           (flag "-p"
