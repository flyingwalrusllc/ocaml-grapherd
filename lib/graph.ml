    ...
    if tries < max_attempts then
      match
        ...
        with Invalid_argument _ ->
          ...
          | Ok g -> add_int g label a (tries + 1) 
          | Error _ -> Error (Error.of_string "unable to resize graph")
      with
      | Ok z -> Ok z
      | Error _ ->
         add_int graph label a (tries+1)
    else
      Error (Error.of_string "To many attempts")
  in
...
let%test_module "graph" =
  ( module struct
    ...
    let extract_or_raise graph_res msg =
      ...
      | Ok g -> g
      | Error _ -> raise (Invalid_argument msg)
      
    let graph = extract_or_raise (add initial_graph l20 l50) "add of l50 failed"
    ...
    let graph = extract_or_raise (add graph l1000 l800) "add of l800 to l1000 failed"
              
    let%test "increase capacity" =
      (capacity graph) > 1000
